...
def executeIPP_py(scenario_number,N=6,resolution=0.1, number_of_iterations=20, save_fig_flag=True,):
    ...
    for iteration in range(number_of_iterations):
        ...
        positions_plots = [main_axes.plot(positions_array[:iteration+1, 0, i], positions_array[:iteration+1, 1, i], color=ROBOT_COLOR[(i) % len(ROBOT_COLOR)])[0] for i in range(N)]
        ...
        plot_mean_and_var(X,Y,means_min,means_max,max_var,min_var,pred_mean,pred_std.reshape(pred_mean.shape),pred_mean_fig=pred_mean_fig,pred_var_fig=pred_var_fig)
        # Eq 9 from paper [1]
