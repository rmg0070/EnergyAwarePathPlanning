...
# partitionFinder
def partitionFinder(ax, robotsPositions, envSize_X, envSize_Y, resolution, densityArray,wts):
    hull_figHandles = []
...
def plot_mean_and_var(X,Y,mean_min,mean_max,max_var,min_var,pred_mean,pred_var,pred_mean_fig=None,pred_var_fig=None):
    ...
    # contour_plot_var = ax_2d_var.contourf(X, Y, pred_var.reshape(X.shape[0],X.shape[1]), cmap='Reds',vimn)
...
def updated_weights_ratio(weights,energyspent,intial_energy):
    changed_weights = []
    # print("energy spent:",energyspent)
    for i in range(len(weights)):
        weight_change = 0
        for j in range(len(weights)):
            if i != j:
                energy_ratio = (energyspent[j]/energyspent[i])*(intial_energy[i]/intial_energy[j])
                weight_ratio = (weights[i]/weights[j])
                weight_diff = energy_ratio - weight_ratio
                weight_change = weight_change + weight_diff
        # if i==2 or i==4:
        #     # print("weight change ",weight_change)
        changed_weights.append(round(weight_change/len(weights),4))
