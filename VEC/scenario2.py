...
def executeIPP_py(scenario_number,N=6,resolution=0.1, number_of_iterations=20, save_fig_flag=True,):
    ...
    for iteration in range(number_of_iterations):
        print(f"energy of robots {current_robot_energy}")
        alive_robots = []
        ...
        C_x, C_y , cost, area, global_hull_figHandles, global_hull_textHandles,locationIdx,locationx = partitionFinder(main_axes,current_robotspositions_mod, [x_min,x_max], [y_min,y_max], resolution,surrogate_mean.reshape(Z_phi.shape)) 
        train_X = np.transpose(current_robotspositions)
        Z_phi_current_pos = np.vectorize(lambda x, y: density_function(x, y, means_phi, variances))(current_robotspositions[0,:], current_robotspositions[1,:])+np.random.uniform(-2,2,size=N)
        ...
        centroid = current_robotspositions.copy()

        # print(np.array([C_x,C_y]))
        for robot in range(len(alive_robots)):
            
            centroid[0][alive_robots[robot]]=C_x[robot]
            centroid[1][alive_robots[robot]]=C_y[robot]
        # print(current_robotspositions)
        # print(f"centorid {centroid} sampling {sampling_goal}" )
        ...
        for bot in range(N):
            ...
            coeff_recharge[iteration,bot] = 1.0 - coeff_sampling[iteration,bot]
        ...
        current_robotspositions = copy.deepcopy((1-r_t)*centroid  + (r_t*sampling_goal))
